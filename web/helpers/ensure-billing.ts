...
async function hasActivePayment(session: Session, { chargeName, interval }: IBillingOptions) {
  ...
  if (isRecurring(interval)) {
    ...
    do {
      ...
      purchases = oneTimePurchaseResponse.body.data.currentAppInstallation.oneTimePurchases
...
async function requestPayment(
  ...
  const client = new shopify.clients.Graphql({ session })
  const returnUrl = `https://${shopify.config.hostName}?shop=${session.shop}&host=${Buffer.from(
    `${session.shop}/admin`
  ).toString('base64')}`
